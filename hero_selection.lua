local selectutil =require(GetScriptDirectory().."/select_util")
...
function Think()
	radiantSlots = GetTeamPlayers(TEAM_RADIANT);
	direSlots = GetTeamPlayers(TEAM_DIRE);
	maxPlayerID = direSlots[#direSlots];
...
function PickHero(slot)
	if IsPlayerBot(slot) then
		SelectHero(slot,GetRandomHero());
	end
end
...
function GetRandomHero()
  ...
  for slot, hero in pairs(picks) do
	...
	local num = RandomInt(1, #requiredHeroes);
	hero = requiredHeroes[num];
	if (requiredHeroes[1] == nil) then
		hero = allBotHeroes[RandomInt(1, #allBotHeroes)];
	else
		table.remove(requiredHeroes,num);
	end
